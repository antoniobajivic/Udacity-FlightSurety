...
contract FlightSuretyApp {
    ...
    */
    ...
    modifier requireAirlineIsNotRegistered(address airline) {
        require(!flightSuretyData.getAirlineRegisterStatus(airline), "Airline is already registered");
        _;
    }

    modifier requireAirlineIsNotFunded(address airline) {
        require(!flightSuretyData.getAirlineFundedStatus(airline), "Airline is already funded");
        _;
    }

    modifier requireAirlineIsRegistered(address airline) {
        require(flightSuretyData.getAirlineRegisterStatus(airline), "Airline is not registered");
        _;
    }

    modifier requireAirlineIsFunded(address airline) {
        require(flightSuretyData.getAirlineFundedStatus(airline), "Airline is not funded");
        _;
    }
    ...
    modifier requireFlightIsNotRegistered(bytes32 flightKey) {
        require(!flightSuretyData.getFlightRegisterStatus(flightKey), "Flight is already registered");
        _;
    }

    modifier requireFlightIsRegistered(bytes32 flightKey) {
        require(flightSuretyData.getFlightRegisterStatus(flightKey), "Flight is not registered");
        _;
    }

    modifier requireFlightIsNotLanded(bytes32 flightKey) {
        require(!flightSuretyData.getFlightCodeStatus(flightKey), "Flight is not registered");
        _;
    }

    modifier requireFlightIsNotInsured(bytes32 flightKey, address passenger) {
        require(!flightSuretyData.getFlightInsurance(flightKey, passenger), "Flight is already insured");
        _;
    }

    modifier requireLowerInsuranceValue() {
        require(msg.value <= MAX_INSURANCE_VALUE, "Value is higher than max insurance plan");
        _;
    }
    ...
    */
    constructor
                                (
                                    address payable flightSuretyDataContract
                                ) 
                                public 
    {
    ...
    */   
    ...
    {
        if (flightSuretyData.getRegisteredAirlineNum() <= AIRLINE_VOTING_THRESHOLD) {
            flightSuretyData.registerAirline(airline, msg.sender);
            return (success, 0, flightSuretyData.getRegisteredAirlineNum());
        } else {
            ...
            require(!doubleVote, "You cannot vote for the same airline twice");
            pendingAirlines[airline].push(msg.sender);
            if (pendingAirlines[airline].length >= flightSuretyData.getRegisteredAirlineNum().div(AIRLINE_REGISTRATION_REQUIURED_VOTES)) {
                flightSuretyData.registerAirline(airline, msg.sender);
                return (true, pendingAirlines[airline].length, flightSuretyData.getRegisteredAirlineNum());
            }
            return (false, pendingAirlines[airline].length, flightSuretyData.getRegisteredAirlineNum());
        }
    }
    ...
    */  
    ...
    function fund()
        ...
        returns (bool)
        {
            address(uint256(address(flightSuretyData))).transfer(
...
contract FlightSuretyData {
  ...
  function getAirlineRegisterStatus(address airline) public view returns (bool);

  function getAirlineFundedStatus(address airline) public view returns (bool);

  function getFlightRegisterStatus(bytes32 flightKey) public view returns (bool);

  function getFlightCodeStatus(bytes32 flightKey) public view returns (bool);

  function getFlightInsurance(bytes32 flightKey, address passenger) public view returns (bool);
